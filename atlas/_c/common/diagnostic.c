...
static void cct_diagnostic_autoconfigure_colors(void) {
    ...
    g_colors_enabled = isatty(fileno(stderr)) != 0;
    g_colors_set = true;
}
...
static void cct_diag_emit_snippet(const cct_diagnostic_t *diag) {
    if (!diag || !diag->file_path || !diag->line) {
        return;
    }
    ...
    if (diag->column > 1) {
        ...
        if (marker_indent > src_len) {
            marker_indent = src_len;
        }
    }
...
void cct_error_with_suggestion(
    const char *file,
    u32 line,
    u32 column,
    const char *message,
