...
static bool sp_is_known_top_level_key(const char *key) {
    return key
           && (strcmp(key, "format") == 0 || strcmp(key, "sigilo_scope") == 0
               || strcmp(key, "visual_engine") == 0 || strcmp(key, "sigilo_style") == 0
               || strcmp(key, "semantic_hash") == 0 || strcmp(key, "system_hash") == 0);
}
...
static bool sp_lookup_duplicate_key(
    ...
    for (size_t i = 0; i < doc->entry_count; i++) {
        ...
        if (strcmp(kv->section ? kv->section : "", s) == 0 && strcmp(kv->key, key) == 0) {
            return true;
        }
    }
    return false;
...
static cct_sigil_web_route_t *sp_ensure_web_route(cct_sigil_document_t *doc, u32 section_index) {
    ...
    for (size_t i = 0; i < doc->web_routes_count; i++) {
        if (doc->web_routes[i].section_index == section_index) {
            return &doc->web_routes[i];
        }
    }
    if (doc->web_routes_count >= doc->web_routes_capacity) {
        size_t next = doc->web_routes_capacity == 0 ? 4 : doc->web_routes_capacity * 2;
        cct_sigil_web_route_t *grown =
            (cct_sigil_web_route_t *)realloc(doc->web_routes, next * sizeof(*grown));
        if (!grown) {
            return NULL;
        }
        memset(
            grown + doc->web_routes_capacity,
            0,
            (next - doc->web_routes_capacity) * sizeof(*grown)
        );
...
static void sp_capture_extended_fields(cct_sigil_document_t *doc) {
    ...
    for (size_t i = 0; i < doc->entry_count; i++) {
        ...
        if (strncmp(section, "web_route.", 10) == 0) {
            u64 idx64 = 0;
            if (!sp_parse_u64(section + 10, &idx64)) {
                continue;
            }
            ...
            if (strcmp(key, "route_id") == 0) {
                sp_assign_dup(&route->route_id, value);
            } else if (strcmp(key, "method") == 0) {
                sp_assign_dup(&route->method, value);
            } else if (strcmp(key, "path") == 0) {
                sp_assign_dup(&route->path, value);
            } else if (strcmp(key, "route_name") == 0) {
                sp_assign_dup(&route->route_name, value);
            } else if (strcmp(key, "handler") == 0) {
                sp_assign_dup(&route->handler, value);
            } else if (strcmp(key, "module") == 0) {
                sp_assign_dup(&route->module, value);
            } else if (strcmp(key, "group") == 0) {
                sp_assign_dup(&route->group, value);
            } else if (strcmp(key, "middleware") == 0) {
                sp_assign_dup(&route->middleware, value);
            } else if (strcmp(key, "path_params") == 0) {
                sp_assign_dup(&route->path_params, value);
            } else if (strcmp(key, "route_hash") == 0) {
                sp_assign_dup(&route->route_hash, value);
            } else if (strcmp(key, "source_origin") == 0) {
                sp_assign_dup(&route->source_origin, value);
            }
        }
    }
}
...
static void sp_capture_known_field(
    cct_sigil_document_t *doc,
    const char *section,
    const char *key,
    const char *value
...
const char *cct_sigil_diag_kind_str(cct_sigil_diag_kind_t kind) {
    switch (kind) {
        case CCT_SIGIL_PARSE_OK:
            return "ok";
        case CCT_SIGIL_PARSE_SYNTAX:
            return "syntax";
        case CCT_SIGIL_PARSE_TYPE:
            return "type";
        case CCT_SIGIL_PARSE_MISSING_REQUIRED:
            return "missing_required";
        case CCT_SIGIL_PARSE_DUPLICATE_KEY:
            return "duplicate_key";
        case CCT_SIGIL_PARSE_UNKNOWN_FIELD:
            return "unknown_field";
        case CCT_SIGIL_PARSE_DEPRECATED_FIELD:
            return "deprecated_field";
        case CCT_SIGIL_PARSE_SCHEMA_MISMATCH:
            return "schema_mismatch";
        case CCT_SIGIL_PARSE_IO:
            return "io";
        default:
            return "unknown";
    }
    ...
    while (cursor && *cursor) {
        ...
        *eq = '\0';
        ...
        if (sp_lookup_duplicate_key(out_doc, section, key)) {
            char msg[256];
            snprintf(msg, sizeof(msg), "duplicate key '%s' in section [%s]", key, section ? section : "");
            (void)sp_push_diag(
                &out_doc->diagnostics,
                CCT_SIGIL_DIAG_ERROR,
        ...
        if ((section == NULL || section[0] == '\0') && !sp_is_known_top_level_key(key)) {
            char msg[256];
            snprintf(msg, sizeof(msg), "unknown top-level field '%s'", key);
            (void)sp_push_diag(
                &out_doc->diagnostics,
                sp_mode_is_strict_contract(mode) ? CCT_SIGIL_DIAG_ERROR : CCT_SIGIL_DIAG_WARNING,
        ...
        if (sp_is_deprecated_top_level_key(key)) {
            char msg[256];
            snprintf(msg, sizeof(msg), "deprecated top-level field '%s'", key);
            (void)sp_push_diag(
                &out_doc->diagnostics,
...
bool cct_sigil_validate(cct_sigil_document_t *doc, cct_sigil_parse_mode_t mode) {
    ...
    if (!cct_sigil_validate_collect(doc, mode, &result)) {
        return false;
    }

    for (size_t i = 0; i < result.count; i++) {
        const cct_sigil_validation_issue_t *issue = &result.items[i];
        (void)sp_push_diag(
            &doc->diagnostics,
            issue->level,
            issue->kind,
    ...
    return !cct_sigil_document_has_errors(doc);
}
...
void cct_sigil_document_dispose(cct_sigil_document_t *doc) {
    ...
    for (size_t i = 0; i < doc->web_routes_count; i++) {
        cct_sigil_web_route_t *route = &doc->web_routes[i];
        free(route->route_id);
        free(route->method);
        free(route->path);
        ...
        free(route->route_hash);
        free(route->source_origin);
    }
    free(doc->web_routes);

    for (size_t i = 0; i < doc->entry_count; i++) {
        cct_sigil_kv_t *kv = &doc->entries[i];
        free(kv->section);
        free(kv->key);
        free(kv->value);
    }
