...
static bool sv_push_issue(
    ...
    if (out->count >= CCT_SIGIL_VALIDATE_MAX_ISSUES) {
        return false;
    }
...
static const cct_sigil_kv_t *sv_find_entry(
    ...
    for (size_t i = 0; i < doc->entry_count; i++) {
        ...
        if (strcmp(kv->section ? kv->section : "", s) != 0) {
            continue;
        }
        if (strcmp(kv->key ? kv->key : "", key) != 0) {
            continue;
        }
        found = kv;
    }
...
static void sv_validate_numeric_key_if_present(
    ...
    u64 parsed = 0;
    if (sv_parse_u64(kv->value, &parsed)) {
        return;
    }
    cct_sigil_diag_level_t lvl =
        sv_mode_is_strict_contract(mode) ? CCT_SIGIL_DIAG_ERROR : CCT_SIGIL_DIAG_WARNING;
    char msg[220];
    snprintf(
        msg,
        sizeof(msg),
...
static void sv_validate_consistency(
    ...
    const cct_sigil_kv_t *summary_module_count =
        sv_find_entry(doc, "module_structural_summary", "module_count");
    if (top_module_count && summary_module_count) {
        u64 top = 0;
        u64 sec = 0;
        if (sv_parse_u64(top_module_count->value, &top)
            && sv_parse_u64(summary_module_count->value, &sec) && top != sec) {
            (void)sv_push_issue(
                out,
                lvl,
                CCT_SIGIL_PARSE_TYPE,
    ...
    const cct_sigil_kv_t *sec_mod_res =
        sv_find_entry(doc, "module_structural_summary", "module_resolution_status");
...
static bool sv_route_method_ok(const char *method) {
    return method && (strcmp(method, "GET") == 0 || strcmp(method, "POST") == 0
                      ...
                      || strcmp(method, "OPTIONS") == 0 || strcmp(method, "ANY") == 0
                      || strcmp(method, "CUSTOM") == 0 || strcmp(method, "UNKNOWN") == 0);
}
...
static void sv_validate_web_routes(
    ...
    if (!doc || !doc->has_web_routes) {
        return;
    }
    cct_sigil_diag_level_t lvl =
        sv_mode_is_strict_contract(mode) ? CCT_SIGIL_DIAG_ERROR : CCT_SIGIL_DIAG_WARNING;

    if (doc->web_topology_hash && doc->web_topology_hash[0] != '\0'
        && !sv_is_hash_hex_16(doc->web_topology_hash)) {
        (void)sv_push_issue(
            out,
            lvl,
            CCT_SIGIL_PARSE_TYPE,
...
bool cct_sigil_validate_collect(
    ...
    sv_validate_numeric_key_if_present(
        doc,
        mode,
        out_result,
